		...
		if (request.getParameter("calcular")!=null) {
			...
			if (request.getParameter("Consultar_cliente")!=null) {
				...
				if (clien!=null) {
					...
					response.sendRedirect("Venta.jsp?cedula_cliente="+cedulaCliente+"&&nombre_cliente="+
							URLEncoder.encode(nombreCliente, StandardCharsets.UTF_8)+"&&cedula_usuario="+cedulaUsuario+"&&nombreUsuario="+nombreUsuario);
					
				}else {
					response.sendRedirect("Venta.jsp?men=Cliente no existe. ");
				}
			}
				...
				if (request.getParameter("Conproducto1")!=null) {
					...
					if (proDTO!=null) {
						...
						response.sendRedirect("Venta.jsp?men=Producto no Existe.");
				...
				if (request.getParameter("confirmar") != null) {
					...
					try {
						...
						String sql= "select max(codigoVenta) from tiendagenerica.ventas";
